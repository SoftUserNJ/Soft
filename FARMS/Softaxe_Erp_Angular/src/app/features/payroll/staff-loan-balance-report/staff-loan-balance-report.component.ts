...
@Component({
  selector: 'app-staff-loan-balance-report',
  templateUrl: './staff-loan-balance-report.component.html',
  styleUrls: ['./staff-loan-balance-report.component.css']
})
export class StaffLoanBalanceReportComponent {
  ...
  StaffLoanBlanaceForm!: FormGroup;

  openReportModal() {
    let form = this.StaffLoanBlanaceForm.value;
    let url = `RptStfLoanBlnce?`;
...
formInit() 
{ 
  this.StaffLoanBlanaceForm = this.fb.group({
...
InputEmployee(event:any) {
  const selectedEmployeeId: number = parseInt(event.target.value, 10);
      ...
      if (selectedEmployee) {
        this.StaffLoanBlanaceForm.patchValue({ empName: selectedEmployee.empy_id });
      }
      else{
...
SelectEmployee(event:any) {
        this.StaffLoanBlanaceForm.patchValue({ empy_id: event.empy_id });
}
