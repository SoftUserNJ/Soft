...
@Component({
  ...
  styleUrls: ['./sales-receipts.component.css'],
})
export class SalesReceiptsComponent {
  ...
  async callOldBalance(code: any) {
    const oldBal = await this.apiService
      ...
      .toPromise();
    if (this.isNew) {
      this.previousBalance = this.com.roundVal((oldBal - this.totalDueAmount).toFixed(2));
    } else {
      this.previousBalance = this.com.roundVal(oldBal > 0 ? 0 : oldBal.toFixed(2));
    }
  }
  ...
  appendData(): void {
    ...
    const accountName = this.AccountHead.find((x) => x.code == form.toAccountCode);
  ...
  editRow(row: any) {
    ...
    this.receiptsForm.get('jobNo')?.patchValue(row.jobNo);
    this.isRowEdit = true;
    this.receiptsForm.get('accountCode').enable();
  }
  ...
  oninputRec(i: any) {
    ...
    if (parseFloat(netAmt) < totalRecAmt) {
      ...
      i.RECAMOUNT = this.com.roundVal(Math.abs(finalValue - i.RECAMOUNT).toFixed(2));
      this.invoiceCalculation();
    }
  }
  ...
  onInputDiscount(event: any) {
    ...
    this.disAmount = this.com.roundVal(((this.grossAmount * event.target.value) / 100).toFixed(2));
    this.disCalculate();
  }
  ...
  async editAccountReceipts(vchNo: any, vchType: any, partyCode: any) {
    try {
      this.com.showLoader();
