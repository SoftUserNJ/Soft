...
export class ApprovalComponent {
  ...
  getUserList() {
    this.apiService
      .getDataById('Auth/GetUsersList', {
        locId: this.locId == 'HO' ? '%' : this.locId,
      })
      .subscribe((data) => {
        this.usersList = data;
      });
  ...
  getVchTypes() {
    this.apiService
      .getDataById('Accounts/GetAllowVchType', { tag: 'approve' })
      .subscribe((data) => {
  ...
  onClickChk(e: any, i: any) {
    if (this.isVerify) {
      if (!e.target.checked) {
        let rmk = prompt('Enter Reason For Un-Approved Voucher');
        if (rmk == '' || rmk == null) {
        ...
        this.approvalVchList[i].RMK = rmk;
      }
    }
  }
