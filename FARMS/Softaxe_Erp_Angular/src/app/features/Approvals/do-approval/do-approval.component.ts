...
export class DOApprovalComponent {
  ...
  GetVoucher(): void {
    ...
    this.apiService.getDataById('Approval/GetDOApprovalList',obj).subscribe((data) => {
  ...
  onClickSave() {
    ...
    try {
      ...
      this.apiService
        .saveData('Approval/UpdateDoStatus', voucher)
        .subscribe((result) => {
  ...
  onViewReport(data: any) {
    let url = '';
    const vchNo = data.VchNO;
    const vchType = 'SP';
    const vchDate = data.VchDate
    const [day, month, year] = vchDate.split('/');
    const formattedDate = `${year}/${month}/${day}`;

    url = `SaleInvoice?VchNoFrom=${vchNo}&VchNoTo=${vchNo}&VchType=${vchType}&fromDate=${formattedDate}&toDate=${formattedDate}&CmpId=${this.auth.cmpId()}&FinId=${this.auth.finId()}&LocId=${this.auth.locId()}`;
