...
@Component({
  selector: 'app-ipe-deduction-report',
  templateUrl: './ipe-deduction-report.component.html',
  styleUrls: ['./ipe-deduction-report.component.css']
})
export class IPEDeductionReportComponent {
  ...
  openReportModal() {
    let form = this.IPEForm.value;
    let url = `RptIncomeEOBIProvDed?`;
...
InputEmployee(event:any) {
  const selectedEmployeeId: number = parseInt(event.target.value, 10);
      ...
      if (selectedEmployee) {
        this.IPEForm.patchValue({ empName: selectedEmployee.empy_id });
      }
      else{
...
SelectEmployee(event:any) {
        this.IPEForm.patchValue({ empy_id: event.empy_id });
}
