$(document).ready(function () {
    ...
    init(); $(document).on('mouseover', function (e) {
        e.stopPropagation(); if ($('body').hasClass('mini-sidebar') && $('#toggle_btn').is(':visible')) {
            var targ = $(e.target).closest('.sidebar, .header-left').length; if (targ) { $('body').addClass('expand-menu'); $('.subdrop + ul').slideDown(); $("#ct-menue-left").show(); } else { $('body').removeClass('expand-menu'); $('.subdrop + ul').slideUp(); $("#ct-menue-left").hide(); }
            return false;
        }
    ...
    $('ul.tabs li').click(function () { var $this = $(this); var $theTab = $(this).attr('id'); console.log($theTab); if ($this.hasClass('active')) { } else { $this.closest('.tabs_wrapper').find('ul.tabs li, .tabs_container .tab_content').removeClass('active'); $('.tabs_container .tab_content[data-tab="' + $theTab + '"], ul.tabs li[id="' + $theTab + '"]').addClass('active'); } }); var customize_link = '<div class="customizer-links">' +
        ...
        '<label for="1" class="checktoggle"><a  href="javascript:void(0);"class="layout-link">checkbox</a> </label>' +
        '</div>' +
        ...
        '<label for="3" class="checktoggle"><a  href="javascript:void(0);"class="layout-link">checkbox</a> </label>' +
        '</div>' +
