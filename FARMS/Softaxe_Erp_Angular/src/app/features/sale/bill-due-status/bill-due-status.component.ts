...
export class BillDueStatusComponent {
  ...
  onViewReport(item: any) {
    ...
    let url = `SaleInvoice?VchNoFrom=${item.INVOICENO}&VchNoTo=${
      item.INVOICENO
    }&VchType=SP&fromDate=${vchDate}&toDate=${vchDate}&CmpId=${this.auth.cmpId()}&FinId=${this.auth.finId()}&LocId=${this.auth.locId()}`;
  ...
  async getInvoice() {
    try {
      ...
      const data = await this.apiService
        .getDataById('Sale/GetBillDueStatus', obj)
        .toPromise();
  ...
  calculateCommission() {
    ...
    const ot = this.orderTakerList.find((x) => x.id == this.orderTakerId);
    var com = this.orderTakerCommission.split('-');

    if (parseFloat(ot.Recovery) > this.totalRecAmount) {
      this.commission = (this.totalRecAmount / 100) * parseFloat(com[0]);
    } else if (parseFloat(ot.Recovery) < this.totalRecAmount) {
      const recAmt = this.totalRecAmount - parseFloat(ot.Recovery);
      let commission1 = (parseFloat(ot.Recovery) / 100) * parseFloat(com[0]);
      let commission2 = (recAmt / 100) * parseFloat(com[1]);
      this.commission = commission1 + commission2;
    }
  }
  ...
  onChangeOrderTaker(event: any) {
    if (event == undefined) {
      this.commission = 0;
      this.orderTakerCommission = '';
      this.orderTaker = '';
      this.searchGrid();
      return;
    }

    this.orderTakerCommission = event.Commission + '-' + event.AboveCommission;
    this.orderTaker = event.userName;
    this.searchGrid();
  ...
  searchGrid(): void {
    ...
    rows.forEach((row: HTMLTableRowElement) => {
      ...
      if (
        row.querySelector('.orderTaker')?.textContent != this.orderTaker &&
        this.orderTaker.length > 0
      ) {
        isShow = false;
      }
