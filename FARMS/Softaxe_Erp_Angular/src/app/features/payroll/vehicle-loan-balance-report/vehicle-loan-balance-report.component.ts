...
@Component({
  selector: 'app-vehicle-loan-balance-report',
  templateUrl: './vehicle-loan-balance-report.component.html',
  styleUrls: ['./vehicle-loan-balance-report.component.css']
})
export class VehicleLoanBalanceReportComponent {
  ...
  VehLoanBlanaceForm!: FormGroup;

  openReportModal() {
    let form = this.VehLoanBlanaceForm.value;
    let url = `RptVehLoanBalance?`;
...
formInit() 
{ 
  this.VehLoanBlanaceForm = this.fb.group({
...
InputEmployee(event:any) {
  const selectedEmployeeId: number = parseInt(event.target.value, 10);
      ...
      if (selectedEmployee) {
        this.VehLoanBlanaceForm.patchValue({ empName: selectedEmployee.empy_id });
      }
      else{
...
SelectEmployee(event:any) {
        this.VehLoanBlanaceForm.patchValue({ empy_id: event.empy_id });
}
