...
export class ReciptsComponent {
  ...
  togglePages() {
    ...
    if (this.isShowPage) {
      this.onClickRefresh();
      this.receiptsForm.get('jobNo')?.patchValue(null);
      this.bankReceipts = false;
      this.vchType = 'CR';
    }
  }
  ...
  getAccountHead() {
    this.apiService
      ...
      this.apiService
      .getDataById('Accounts/GetAccountsList', { module: "Account" })
      .subscribe((data) => {
        this.AccountPaidHead = data;
      });
  }
  ...
  onClickCashReceipts(event: any): void {
    if (this.isNewClick) {
  ...
  onClickBankReceipts(event: any): void {
    if (this.isNewClick) {
      event.preventDefault();
      return;
    }

    this.bankReceipts = true;
    this.vchType = 'BR';
  }
  ...
  async getBankCash() {
    const data = await this.apiService
      ...
      .toPromise();
    this.BackCash = data;
    this.receiptsForm.get('bankCash')?.patchValue(data[0].code);
  }
  ...
  async onChangeAccount(event: any) {
    ...
    const data = await this.apiService
      .getDataById('Sale/InvoiceList', { code: event.code })
      .toPromise();
  ...
  async callOldBalance(code: any) {
    const oldBal = await this.apiService
      .getDataById('Sale/CallOldBalance', { code: code })
      .toPromise();
    if (this.isNew) {
  ...
  onClickNew() {
    ...
    this.isNew = true;
    if (this.vchType == 'CR') {
      this.receiptsForm.get('mainDes')?.patchValue('Cash Receipts');
      this.receiptsForm.get('description')?.patchValue('Cash Receipts');
    } else if (this.vchType == 'BR') {
      this.receiptsForm.get('mainDes')?.patchValue('Bank Receipts');
      this.receiptsForm.get('description')?.patchValue('Bank Receipts');
    }
  ...
  onClickRefresh() {
    ...
    this.addData = [];
    this.resetForm();
    this.receiptsForm.get('sno')?.patchValue(0);
    this.receiptsForm.get('bankCash')?.patchValue(null);
    this.receiptsForm.get('accountCode')?.patchValue(null);
    this.receiptsForm.get('mainDes')?.patchValue('');
    this.totalAmount = 0;
  ...
  oninputDes(event: any) {
    this.receiptsForm.get('mainDes')?.patchValue(event.target.value);
  }

  appendData(): void {
    let form = this.receiptsForm.value;
    ...
    if (form.accountCode == form.toAccountCode) {
      this.toast.warning('Paid To Account Selection Wrong....!');
      return;
    }
    ...
    const accountName = this.AccountPaidHead.find(
      (x) => x.code == form.toAccountCode
    );
    form.toAccountName = accountName != undefined ? accountName.name : '';
    ...
    if (this.isRowEdit) {
      ...
      if (index !== -1) {
        ...
        if (this.tag != 'Account') {
          this.receiptsForm.get('accountCode').disable();
        }
        return;
      }
    }
    ...
    if (this.tag != 'Account') {
      if (this.addData.length > 0) {
        ...
        this.receiptsForm.get('accountCode').enable();
      }
    }
    //this.calculation();
  }
  ...
  resetForm() {
    ...
    if (this.tag == 'Account') {
      this.receiptsForm.get('accountCode')?.patchValue(null);
      this.receiptsForm.get('toAccountCode')?.patchValue(null);
    }

    this.receiptsForm.get('amount')?.patchValue('');
    this.receiptsForm.get('description')?.patchValue('');
    this.receiptsForm.get('chqNo')?.patchValue('');
    this.receiptsForm.get('chqDate')?.patchValue(new Date());
    this.receiptsForm.get('accountCode').enable();
    this.isRowEdit = false;
  }

  editRow(row: any) {
    this.receiptsForm.get('sno')?.patchValue(row.sno);
    this.receiptsForm.get('accountCode')?.patchValue(row.accountCode);
    this.receiptsForm.get('toAccountCode')?.patchValue(row.toAccountCode);
    this.receiptsForm.get('amount')?.patchValue(row.amount);
    this.receiptsForm.get('description')?.patchValue(row.description);
    this.receiptsForm.get('chqNo')?.patchValue(row.chqNo);
    const splitDate = row.chqDate.split('/');
    const chqDate = new Date(splitDate[2], splitDate[1] - 1, splitDate[0]);
    this.receiptsForm.get('chqDate')?.patchValue(chqDate);
    this.isRowEdit = true;
    this.receiptsForm.get('accountCode').enable();
  }
  ...
  onClickSaveDis() {
    const obj = {
      ...
      partyName: this.AccountHead.find(
        (x) => x.code === this.receiptsForm.get('accountCode').value
      ).name,
    ...
    try {
      this.com.showLoader();
      this.apiService.saveObj('Sale/SaveDiscount', obj).subscribe((result) => {
        if (result == true || result == 'true') {
          if (this.isNew) {
            this.onChangeAccount({
              code: this.receiptsForm.get('accountCode').value,
            });
          } else {
            const obj = {
              vchNo: this.vchNo,
              vchType: this.vchType,
              partyCode: this.receiptsForm.get('accountCode').value,
              status: 'Receipts',
            };
  ...
  onClickSave(): void {
    ...
    try {
      this.com.showLoader();
      this.lastVchAccount = this.receiptsForm.get('bankCash').value;
      const job = this.JobList.find((x) => x.ID == this.receiptsForm.get('jobNo').value);

      const payment: any[] = this.addData.map((item) => ({
        vchNo: this.vchNo,
        vchType: this.vchType,
        vchDate: this.dp.transform(this.vchDate, 'yyyy-MM-dd'),
        bankCashName: this.BackCash.find((x) => x.code == this.receiptsForm.get('bankCash').value).name,
        bankCash: this.receiptsForm.get('bankCash').value,
        mainDesc: this.receiptsForm.get('mainDes').value,
        jobNo: this.receiptsForm.get('jobNo').value ?? 0,
        ...
        toAccountHead: item.toAccountCode,
        toAccountHeadName: item.toAccountName,
      ...
      this.apiService
        .saveData('Sale/SaveUpdatePR', { payment, invoice })
        .subscribe(async (r) => {
  ...
  async editByVchNo(e: any){
    ...
    if (e.target.value) {
        this.ntimer = setTimeout(async () => {
          this.addData = [];
            await this.editAccountReceipts(e.target.value, this.vchType, false);
        }, this.ntimeout);
    }
  }

  async editAccountReceipts(vchNo: any, vchType: any,isToggle: boolean) {
    ...
    try {
      ...
      const data = await this.apiService
        .getDataById('Sale/EditPR', obj)
        .toPromise();

        if(data.length == 0){
          this.toast.info("Voucher Not Found");
          this.onClickRefresh();
          this.com.hideLoader();
          return;
        }
      ...
      data.forEach((item: any) => {
        this.receiptsForm.get('bankCash')?.patchValue(item.BANKCASHCODE);
        this.receiptsForm.get('accountCode')?.patchValue(item.PARTYCODE);
        this.receiptsForm.get('jobNo')?.patchValue(item.JOBID);
        this.bankcashtName = item.BANKCASH;
        this.receiptsForm.get('mainDes')?.patchValue(item.MAINDESCRIPTION);
        let form = this.receiptsForm.value;
        form.sno = this.addData.length + 1;
        form.accountName = item.NAMES;
        form.accountCode = item.PARTYCODE;
        form.toAccountName = item.TONAME;
        form.toAccountCode = item.TOCODE;
  ...
  async editPartyData(obj: any) {
    const inv = await this.apiService
      .getDataById('Sale/EditPartyPR', obj)
      .toPromise();
  ...
  deleteAccountReceipts(vchNo: any, vchType: any): void {
