using DevExpress.Pdf.Native.BouncyCastle.Math;

namespace SoftaxeERP_API.VM
{
    public class PurchaseCorrectionVM
    {
        public string vchType { get; set; }
        public int vchNo { get; set; }
        public int gpino { get; set; }
        public string location { get; set; }
        public string vehicleNo { get; set; }
        public string biltyNo { get; set; }
        public string grnDate { get; set; }
        public string freightDD { get; set; }
        public int freight { get; set; }

        public int pono { get; set; }


        public bool chkMinWeight { get; set; }
        public string weighttype { get; set; }
        public string partyMain { get; set; }
        public string partySub { get; set; }
        public string subParty { get; set; }
        public string itemMain { get; set; }
        public string itemSub { get; set; }
        public int godown { get; set; }
        public int uom { get; set; }
        public string bagsType { get; set; }
        public int sbags { get; set; }
        public int gross { get; set; }
        public int tare { get; set; }
        public int bagsWt { get; set; }
        public float expWt { get; set; }
        public int standardWt { get; set; }
        public string retStat { get; set; }
        public int sTax { get; set; }
        public int iTax { get; set; }
        public int net { get; set; }
        public string remarks { get; set; }
        public int manualGpiNo { get; set; }
        public int weight { get; set; }
        public int wbCharge { get; set; }
        public float rate { get; set; }
        public int calcLab { get; set; }
        public int stockWt { get; set; }
        public int labStk { get; set; }
        public int biltyWt { get; set; }
        public int grossWt { get; set; }
        public int grossWt2 { get; set; }
        public int secWt { get; set; }
        public int firstWt { get; set; }
        public int payableWt { get; set; }
        public int labParty { get; set; }
        public int netWt { get; set; }
        public int tareWt { get; set; }
        public string bagsTypeDDS1 { get; set; }
        public float bagWt1 { get; set; }
        public int bag1 { get; set; }
        public string bagsTypeDDS2 { get; set; }
        public float bagWt2 { get; set; }
        public int bag2 { get; set; }
        public string bagsTypeDDS3 { get; set; }
        public float bagWt3 { get; set; }
        public int bag3 { get; set; }

        public int id { get; set; }
    }


    //public class PurchaseCorrectionVM
    //{
    //    public int? CmpId { get; set; }

    //    public string LocId { get; set; }

    //    public int? FinId { get; set; }

    //    public string VchType { get; set; }

    //    public int? VchNo { get; set; }

    //    public string VehicleNo { get; set; }

    //    public int? FirstWeight { get; set; }

    //    public int? SecWeight { get; set; }

    //    public string PovchType { get; set; }

    //    public double? Final { get; set; }

    //    public int? Pono { get; set; }

    //    public int? Gpno { get; set; }

    //    public DateTime? VchDate { get; set; }

    //    public string Dmcode { get; set; }

    //    public string Code { get; set; }

    //    public string Mcode { get; set; }

    //    public string SubCode { get; set; }

    //    public string Descrp { get; set; }

    //    public double? Debit { get; set; }

    //    public double? Credit { get; set; }

    //    public decimal? Tucks { get; set; }

    //    public string MsCode { get; set; }

    //    public string ChqNo { get; set; }

    //    public DateTime? ChqDate { get; set; }

    //    public decimal? TaxP { get; set; }

    //    public decimal? Qty { get; set; }

    //    public double? Bags { get; set; }

    //    public double? Rate { get; set; }

    //    public int? EventNo { get; set; }

    //    public string Uid { get; set; }

    //    public string Uom { get; set; }

    //    public double? DivUom { get; set; }

    //    public string BagsType { get; set; }

    //    public int? Freight { get; set; }

    //    public string FreightType { get; set; }

    //    public double? Sqty { get; set; }

    //    public string Wtype { get; set; }

    //    public bool? Reject { get; set; }

    //    public int? Dono { get; set; }

    //    public int? TvchNo { get; set; }

    //    public int? Printed { get; set; }

    //    public int? PrintedBy { get; set; }

    //    public int? RpBy { get; set; }

    //    public bool? SendOn { get; set; }

    //    public bool? RejectedBy { get; set; }

    //    public DateTime? Wdate { get; set; }

    //    public DateTime? DateIn { get; set; }

    //    public DateTime? DateOut { get; set; }

    //    public TimeSpan? TimeIn { get; set; }

    //    public TimeSpan? TimeOut { get; set; }

    //    public int? LabDed { get; set; }

    //    public int? LabDedP { get; set; }

    //    public int? BagsDed { get; set; }

    //    public double? PayableWt { get; set; }

    //    public double? PayableWt1 { get; set; }

    //    public double? Dono1 { get; set; }

    //    public long Id { get; set; }

    //    public double? Labdeds { get; set; }

    //    public string SubName { get; set; }

    //    public string SubParty { get; set; }

    //    public int? MedAmt { get; set; }

    //    public string MedName { get; set; }

    //    public bool? Bnf { get; set; }

    //    public int? Bg1 { get; set; }

    //    public int? Bg2 { get; set; }

    //    public int? Bg3 { get; set; }

    //    public int? Bg4 { get; set; }

    //    public double? Rt1 { get; set; }

    //    public double? Rt2 { get; set; }

    //    public double? Rt3 { get; set; }

    //    public double? Rt4 { get; set; }

    //    public double? Brate { get; set; }

    //    public int? Sno { get; set; }

    //    public string Imported { get; set; }

    //    public string Wb { get; set; }

    //    public string StockType { get; set; }

    //    public string BilltyNo { get; set; }

    //    public string TransporterName { get; set; }

    //    public string Godowns { get; set; }

    //    public string Godowns2 { get; set; }

    //    public string Godowns3 { get; set; }

    //    public string Cmb1 { get; set; }

    //    public string Cmb2 { get; set; }

    //    public string Cmb3 { get; set; }

    //    public int? Sbags { get; set; }

    //    public int? Bags1 { get; set; }

    //    public int? Bags2 { get; set; }

    //    public int? Bags3 { get; set; }

    //    public int? Qty1 { get; set; }

    //    public int? Qty2 { get; set; }

    //    public int? Qty3 { get; set; }

    //    public double? Ded1 { get; set; }

    //    public double? Ded2 { get; set; }

    //    public double? Ded3 { get; set; }

    //    public string Bc1 { get; set; }

    //    public string Bc2 { get; set; }

    //    public string Bc3 { get; set; }

    //    public bool? MiniWt { get; set; }

    //    public double? ExpWt { get; set; }

    //    public bool? Unloading { get; set; }

    //    public string BagsSize { get; set; }

    //    public double? TaxRate { get; set; }

    //    public string InwardType { get; set; }

    //    public string MatType { get; set; }

    //    public decimal? SalesTax { get; set; }

    //    public int? Fed { get; set; }

    //    public decimal? SalesTaxrate { get; set; }

    //    public int? Fedrate { get; set; }

    //    public string DoVchType { get; set; }

    //    public string LocalImport { get; set; }

    //    public string Location { get; set; }

    //    public string Remarks { get; set; }

    //    public string FarmFlockname { get; set; }

    //    public double? ArrivalNo { get; set; }

    //    public double? Freight1 { get; set; }

    //    public decimal? Sent { get; set; }

    //    public bool? A { get; set; }

    //    public bool? T { get; set; }

    //    public int? Gross { get; set; }

    //    public int? Tare { get; set; }

    //    public string LocIdN { get; set; }

    //    public int? Commission { get; set; }

    //    public double? Whtax { get; set; }

    //    public double? Whtaxamt { get; set; }

    //    public double? Comm { get; set; }

    //    public string CommType { get; set; }

    //    public double? OtherExP { get; set; }

    //    public int? Worked { get; set; }

    //    public string PartyName { get; set; }

    //    public string ItemName { get; set; }

    //    public int? LifeDays { get; set; }

    //    public string BrandName { get; set; }

    //    public bool? Fwm { get; set; }

    //    public bool? Swm { get; set; }

    //    public DateTime? Pddate { get; set; }

    //    public int? CloudFlag { get; set; }

    //    public int? Vaprove { get; set; }

    //    public string Shahzad { get; set; }

    //    public DateTime? EntryDate { get; set; }

    //    public double? Dldebit { get; set; }

    //    public double? Dlcredit { get; set; }

    //    public double? DirhamDebit { get; set; }

    //    public double? DirhamCredit { get; set; }

    //    public double? Advance { get; set; }

    //    public string ExtraItem { get; set; }

    //    public double? Amount { get; set; }

    //    public double? FurtherTax { get; set; }

    //    public double? FurtherTaxAmt { get; set; }

    //    public string Country { get; set; }

    //    public string Country1 { get; set; }

    //    public string Port { get; set; }

    //    public string Port1 { get; set; }

    //    public string BookingNo { get; set; }

    //    public string BookingNo1 { get; set; }

    //    public int? CurrencyId { get; set; }

    //    public int? GodownId { get; set; }

    //    public int? RackId { get; set; }

    //    public int? ShelfId { get; set; }

    //    public DateTime? ExpiryDate { get; set; }

    //    public DateTime? DueDate { get; set; }

    //    public int? DeliveryBoyId { get; set; }

    //    public int? OrderTakerId { get; set; }

    //    public decimal? Discount { get; set; }

    //    public decimal? DiscountAmt { get; set; }

    //    public decimal? OtherCredit { get; set; }

    //    public decimal? BillAmount { get; set; }

    //    public decimal? NetAmount { get; set; }

    //    public decimal? ReturnQty { get; set; }

    //    public decimal? Shipment { get; set; }

    //    public decimal? RecAmount { get; set; }

    //    public string Terms { get; set; }

    //    public decimal? ProductDiscount { get; set; }

    //    public decimal? ProductDiscountAmt { get; set; }

    //    public int? PointId { get; set; }

    //    public int? ShiftId { get; set; }

    //    public double? Pcsqty { get; set; }

    //    public int? ToGodown { get; set; }

    //    public int? ToRackno { get; set; }

    //    public int? ToShelfno { get; set; }

    //    public bool? Deposit { get; set; }

    //    public bool? Cleared { get; set; }

    //    public bool? Bounced { get; set; }

    //    public int? CostcentreId { get; set; }

    //    public int? JobNo { get; set; }

    //    public DateTime? Entrydate1 { get; set; }

    //    public double? Frgtax { get; set; }

    //    public double? Frgded { get; set; }

    //    public bool? WorkDone { get; set; }
    //}
}
