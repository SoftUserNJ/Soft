...
@Component({
  selector: 'app-purchase-recipts',
  templateUrl: './purchase-recipts.component.html',
  styleUrls: ['./purchase-recipts.component.css'],
})
export class PurchaseReciptsComponent {
  ...
  getAccountHead() {
    this.apiService
      .getDataById('Accounts/GetAccountsList', { module: 'supplier' })
      .subscribe((data) => {
  ...
  onClickCashRecepits(event: any): void {
    if (this.isNewClick) {
  ...
  onClickBankRecepits(event: any): void {
    if (this.isNewClick) {
      event.preventDefault();
      return;
    }
  ...
  onClickRefresh() {
    ...
    this.receiptsForm.get('bankCash')?.patchValue(undefined);
  ...
  resetForm() {
    this.receiptsForm.get('sno')?.patchValue(0);
    this.receiptsForm.get('accountCode')?.patchValue(undefined);
  ...
  onClickSave(): void {
    ...
    try {
      ...
      this.apiService
        .saveData('Purchase/SaveUpdatePR', { payment })
        .subscribe((result) => {
          if (result == true || result == 'true') {
            this.toast.success('Save Successfully');
            this.isPrint = true;
  ...
  editAccountRecepits(vchNo: any, vchType: any): void {
    this.isDisabled = false;
    this.isNewClick = true;
    this.isNew = false;
    ...
    try {
      ...
      this.apiService.getDataById('Purchase/EditPR', obj).subscribe((data) => {
  ...
  deleteAccountRecepits(vchNo: any, vchType: any): void {
    const confirmDelete = confirm('Are you sure you want to delete this item?');
    ...
    try {
      ...
      this.apiService.deleteData('Purchase/DeletePR', obj).subscribe({
        next: (data) => {
