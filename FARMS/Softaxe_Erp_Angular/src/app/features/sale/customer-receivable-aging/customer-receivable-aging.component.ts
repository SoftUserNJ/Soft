...
@Component({
  selector: 'app-customer-receivable-aging',
  templateUrl: './customer-receivable-aging.component.html',
  styleUrls: ['./customer-receivable-aging.component.css'],
})
export class CustomerReceivableAgingComponent {
  date: Date;
  ...
  onViewReport() {
    ...
    let url = `Aging?FromDate=${date}&ToDate=${date}&FromGroupId=${groupId}&ToGroupId=${groupId1}&FromSubGroupId=${subGroupId}&ToSubGroupId=${subGroupId1}&FromCode=${code}&ToCode=${code1}&AreaWise=${
      ...
      this.locId ?? '%'
    }&filterBy=Customer&LocName=${this.locName}`;
  ...
  GetAging() {
    this.apiService.getData('Sale/GetAging').subscribe((data) => {
  ...
  SaveDays() {
    try {
      ...
      this.apiService.saveObj('Sale/SaveUpdateAging', obj).subscribe((data) => {
