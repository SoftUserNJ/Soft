...
@Component({
  selector: 'app-insurance-loan-balance-report',
  templateUrl: './insurance-loan-balance-report.component.html',
  styleUrls: ['./insurance-loan-balance-report.component.css']
})
export class InsuranceLoanBalanceReportComponent {
  ...
  InsrnceLoanBlanaceForm!: FormGroup;

  openReportModal() {
    let form = this.InsrnceLoanBlanaceForm.value;
    let url = `RptInsurnceLoanBlnce?`;
...
formInit() 
{ 
  this.InsrnceLoanBlanaceForm = this.fb.group({
...
InputEmployee(event:any) {
  const selectedEmployeeId: number = parseInt(event.target.value, 10);
      ...
      if (selectedEmployee) {
        this.InsrnceLoanBlanaceForm.patchValue({ empName: selectedEmployee.empy_id });
      }
      else{
...
SelectEmployee(event:any) {
        this.InsrnceLoanBlanaceForm.patchValue({ empy_id: event.empy_id });
}
}
