using System;
using System.Collections.Generic;

namespace SoftaxeERP_API.Models;

public partial class TblRealSalary
{
    public DateTime? BirthDate { get; set; }

    public string City { get; set; }

    public string Grade { get; set; }

    public DateTime? JoinDate { get; set; }

    public int? Through { get; set; }

    public string CompanyName { get; set; }

    public string Caddress { get; set; }

    public string Cphone { get; set; }

    public int? EmpyId { get; set; }

    public string Name { get; set; }

    public int? DeptId { get; set; }

    public int? DesgId { get; set; }

    public string Fname { get; set; }

    public double? Level2 { get; set; }

    public double? Level4 { get; set; }

    public double? Level6 { get; set; }

    public double? Level3 { get; set; }

    public double? Level5 { get; set; }

    public double? Level7 { get; set; }

    public double? Bsalary { get; set; }

    public double? Gsalary { get; set; }

    public double? Netsalary { get; set; }

    public string Department { get; set; }

    public string Designation { get; set; }

    public double? Ad { get; set; }

    public double? Tel { get; set; }

    public double? Maint { get; set; }

    public double? Nightstay { get; set; }

    public double? Tada { get; set; }

    public double? Pet { get; set; }

    public double? Iother { get; set; }

    public double? Arrears { get; set; }

    public double? Ol { get; set; }

    public double? Sl { get; set; }

    public double? Sli { get; set; }

    public double? Bonus { get; set; }

    public double? Ot { get; set; }

    public decimal? Oh { get; set; }

    public double? Pd { get; set; }

    public double? Itd { get; set; }

    public double? Eobi { get; set; }

    public int? Lv { get; set; }

    public double? Lp { get; set; }

    public double? BsalaryA { get; set; }

    public double? Level2A { get; set; }

    public double? Level3A { get; set; }

    public double? Level4A { get; set; }

    public double? Level5A { get; set; }

    public double? Level6A { get; set; }

    public double? Level7A { get; set; }

    public int? Srno { get; set; }

    public string Acctno { get; set; }

    public string Branchcode { get; set; }

    public double? Banksalary { get; set; }

    public double? Cashsalary { get; set; }

    public int? Location { get; set; }

    public double? Vl { get; set; }

    public double? Vli { get; set; }

    public string Tumbid { get; set; }

    public double? LoanAdjustment { get; set; }

    public double? ArrearsPf { get; set; }

    public double Il { get; set; }

    public double Ili { get; set; }

    public double Pl { get; set; }

    public double Pli { get; set; }
}
